...
![SPM](https://img.shields.io/badge/Package-SPM-red.svg)
...
For example: to generate a password for GitHub, I just enter **Identity** : `github`, **MasterKey** : `password`, and then the system default configuration: `AES256`, and then generate the password:  `eBU6FF-4c0jZs-pb4H`
...
例如：生成GitHub的密码，我只要输入 **Identity**： `github`， **MasterKey**：`password`， 然后系统默认的配置： `AES256`， 然后就生成了密码： `eBU6FF-4c0jZs-pb4H`
...
The project uses **SwiftUI** for all UI, **Swift Package Manager** for dependencies, and **XcodeGen** to generate the Xcode project.

```
project.yml              # XcodeGen project definition (source of truth)
PasswordX.xcodeproj      # Generated by `xcodegen generate`, do not edit by hand
PasswordX/               # Main app (SwiftUI lifecycle, @main App)
PasswordXAuth/           # Auto-fill credential provider extension (SwiftUI hosted)
Shared/                  # Code shared by the app and the extension
PasswordCryptor/         # Local Swift package (core crypto & config model)
└── Package.swift        # depends on CryptoSwift via SPM
```

Third-party dependencies are resolved automatically by SPM (`CryptoSwift`).

```bash
# Install xcodegen: brew install xcodegen
xcodegen generate       # regenerate PasswordX.xcodeproj after editing project.yml or adding files
open PasswordX.xcodeproj
```
...
- **Blowfish is broken.** `BlowfishPasswordCryptor` requests a 65-byte key while CryptoSwift requires 5...56 bytes, so the initializer traps and selecting Blowfish crashes the app. Every other algorithm is covered by tests in `PasswordCryptor/Tests`.

---

## Testing

`PasswordCryptor` ships a test suite that pins the generated passwords and the on-disk config format:

```bash
swift test --package-path PasswordCryptor
```

CI (`.github/workflows/ci.yml`) runs those tests, lints every Swift source with `swift format`, regenerates the project with XcodeGen, and builds both the app and the credential provider extension.
